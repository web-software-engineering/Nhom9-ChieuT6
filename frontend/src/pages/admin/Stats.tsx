  ...
  const fetchData = async () => {
    try {
      ...
      const [revRes, ordRes, topRes] = await Promise.all([
        axios.get("http://localhost:3000/api/stats/revenue", { params: { filter: filterType, start, end } }),
        axios.get("http://localhost:3000/api/stats/orders", { params: { filter: filterType, start, end } }),
        axios.get("http://localhost:3000/api/stats/top-products"),
      ]);
        ...
        {orders.length ? (
          <div className="h-64">
            <Bar
              ...
              data={{
                labels: orders.map(o => o.order_date),
                datasets: [
                  {
                    label: "Số đơn",
                    data: orders.map(o => o.total_orders),
