...
const ProductManagement: React.FC = () => {
  ...
  const handleDeleteSelected = async () => {
    if (selectedProducts.length === 0) return alert("Chưa chọn sản phẩm nào!");
    if (!window.confirm(`Bạn có chắc muốn xóa ${selectedProducts.length} sản phẩm đã chọn?`)) return;
    await Promise.all(selectedProducts.map(id => axios.delete(`${API_URL_PRODUCTS}/${id}`)));
    setSelectedProducts([]);
    fetchProducts();
  };
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between mb-4 gap-2">
        ...
        <div className="flex gap-2">
          <button onClick={() => openModal()} className="bg-green-500 text-white px-5 py-2 rounded hover:bg-green-600">
            + Thêm sản phẩm
          </button>
          <button onClick={handleDeleteSelected} className="bg-red-500 text-white px-5 py-2 rounded hover:bg-red-600">
            Xóa đã chọn
          </button>
        </div>
      </div>
