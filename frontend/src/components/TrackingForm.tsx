...
export default function TrackingForm() {
  ...
  useEffect(() => {
    const savedOrders = JSON.parse(
      localStorage.getItem("office_smart_orders") || "[]",
    );
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    const foundOrders = savedOrders.filter(
      (o: any) => o.orderCode === query || o.phone === query,
    );
        ...
        <div className="mb-6 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Tra cứu bằng Mã vận đơn hoặc Số điện thoại
            </p>
          </div>
        </div>
              ...
              <div className="mt-8 space-y-6 animate-fade-in border-t border-slate-200 pt-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <h3 className="font-bold text-slate-900 flex items-center gap-2 border-b pb-2">
                      <MapPin className="h-5 w-5 text-slate-400" /> Thông tin
                      nhận hàng
                    </h3>
                    <div className="text-sm text-slate-600 space-y-2">
                      <p>
                        <strong>Người nhận:</strong> {orderInfo.receiverName}
                      </p>
                      <p>
                        <strong>Điện thoại:</strong> {orderInfo.phone}
                      </p>
                      <p>
                  ...
                  <div className="space-y-4">
                    <h3 className="font-bold text-slate-900 flex items-center gap-2 border-b pb-2">
                      <Calendar className="h-5 w-5 text-slate-400" /> Chi tiết
                      giao dịch
                    </h3>
                    <div className="text-sm text-slate-600 space-y-2">
                      <p>
                ...
                <div>
                  <h3 className="font-bold text-slate-900 flex items-center gap-2 border-b pb-2 mb-4">
                    <Package className="h-5 w-5 text-slate-400" /> Sản phẩm đã
                    đặt
                  </h3>
                  <ul className="space-y-3">
                    {orderInfo.items.map((item: any, idx: number) => (
                      <li
                        key={idx}
                        className="flex justify-between text-sm bg-slate-50 p-3 rounded-lg border border-slate-100"
                      >
          ...
          <div className="space-y-3">
            {recentOrders.map((order, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-slate-200 p-4 transition hover:border-teal-300 hover:shadow-sm bg-white"
                >
                  <div>
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-xs font-semibold px-2 py-1 rounded-md ${statusInfo.bg} ${statusInfo.color}`}
                    >
                      {statusInfo.text}
                    </span>
                    <button
