...
function MainContent() {
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 pb-12 pt-6 sm:px-6 lg:px-8">
      {currentPage === 'home' && <div className="animate-fade-in"><ProductList /></div>}
      {currentPage === 'cart' && <div className="animate-fade-in"><Cart /></div>}
      {currentPage === 'checkout' && <div className="animate-fade-in"><CheckoutPage /></div>}
      {currentPage === 'tracking' && <div className="animate-fade-in"><TrackingForm /></div>}
      {currentPage === 'users' && <div className="animate-fade-in"><Users /></div>}
    </main>
  );
}
...
=======
          ...
          <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-1 sm:flex-row sm:items-center sm:justify-end">
            <nav className="grid w-full grid-cols-3 gap-2 sm:w-auto sm:flex sm:justify-end">
              {menuItems.map((item) => (
                <button
                  key={item.id}
                  onClick={() => navigate(item.id)}
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              <UserMenu loginHref="/login" variant="store" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-7xl rounded-2xl glass-panel px-6 py-8 sm:px-8">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
          <div>
            <h3 className="text-lg font-bold text-slate-900">Office Smart</h3>
            <p className="mt-3 text-sm leading-relaxed text-slate-600">
              Cung cấp văn phòng phẩm chất lượng cao, tối ưu quy trình đặt hàng
              và giao nhanh toàn quốc.
            </p>
          </div>
          <div>
